...
bool Map::init()
{
    ...
    if (!file.is_open()) {
        ...
        paras = split(temp, ',');
        props.push_back(Prop(HP_PAK, atof(paras[1].c_str()), atof(paras[2].c_str()), 1));

        std::getline(file, temp);
        paras = split(temp, ',');
        props.push_back(Prop(HP_PAK, atof(paras[1].c_str()), atof(paras[2].c_str()), 2));

        std::getline(file, temp);
        paras = split(temp, ',');
        props.push_back(Prop(MP_PAK, atof(paras[1].c_str()), atof(paras[2].c_str()), 2));
        ...
        std::getline(file, temp);
        paras = split(temp, ',');
        props.push_back(Prop(SPD_BUFF, atof(paras[1].c_str()), atof(paras[2].c_str()), 0));

        std::getline(file, temp);
        paras = split(temp, ',');
        props.push_back(Prop(DEF_BUFF, atof(paras[1].c_str()), atof(paras[2].c_str()), 0));
...
void Map::getView(Car * car, std::vector<car_info>& cars) {
    ...
    for (int i = 0; i < props.size(); i++) {
        ...
        if (deta_phi <= 22.5) {
            ...
            if (!be_cover) {
                prop_info prop_i = { props[i].get_type(), Point<int>(int(props[i].get_pos().x),int(props[i].get_pos().y)) };
